...
CSlide::CSlide(LPDIRECT3DDEVICE9 pGraphic_Device)
	: CUi(pGraphic_Device)
{
}

CSlide::CSlide(const CSlide& rhs)
	: CUi(rhs)
{
}

HRESULT CSlide::Initialize_Prototype()
{
...
HRESULT CSlide::Initialize(void* pArg)
{
...
HRESULT CSlide::Initialize_Active()
{
	m_iTexture_Index = 0;
	Default_Set_Size();
	Initialize_Set_Scale_Pos_Rotation(NULL);
...
void CSlide::PriorityTick(_float fTimeDelta)
{
	m_pGraphic_Device->SetRenderState(D3DRS_LIGHTING, false);
}

void CSlide::Tick(_float fTimeDelta)
{
	Set_Divide(1.0f);
	Set_Ui_Pos(0, -170);
...
void CSlide::LateTick(_float fTimeDelta)
{
...
void CSlide::Initialize_Set_Scale_Pos_Rotation(void* pArg)
{
	Set_Ui_Pos(0, -250);
	Set_Divide(1.7f);
...
void CSlide::Initialize_Set_Speed()
{
}

void CSlide::Default_Set_LifeTime()
{
}

void CSlide::Default_Set_Size()
{
	m_UiDesc.m_fSizeX = 840;
	m_UiDesc.m_fSizeY = 1514;
}

POINT CSlide::Get_Texture_Info()
{
...
_float2 CSlide::Lissajous_Curve(_float _fTimeDelta, _float& _fLissajousTime, _float _fPosX, _float _fPosY, _float _fWitth, _float _fHeight, _float _fLagrangianX, _float _fLagrangianY, _float _fPhaseDelta, _float _fLissajousSpeed)
{
...
HRESULT CSlide::Add_Components(void* pArg)
{
...
HRESULT CSlide::Add_Texture(void* pArg)
{
	if (FAILED(Add_Component(LEVEL_STATIC, TEXT("Slide_Textures")
...
CUi* CSlide::Create(LPDIRECT3DDEVICE9 pGraphic_Device)
{
	CSlide* pInstance = new CSlide(pGraphic_Device);

	if (FAILED(pInstance->Initialize(NULL)))
	{
		MSG_BOX(TEXT("Failed to Created : CSlide"));

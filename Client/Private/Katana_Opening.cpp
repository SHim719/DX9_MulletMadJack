...
CKatana_Opening::CKatana_Opening(LPDIRECT3DDEVICE9 pGraphic_Device)
	: CUi(pGraphic_Device)
{
}

CKatana_Opening::CKatana_Opening(const CKatana_Opening& rhs)
	: CUi(rhs)
{
}

HRESULT CKatana_Opening::Initialize_Prototype()
{
...
HRESULT CKatana_Opening::Initialize(void* pArg)
{
	if (E_FAIL == Add_Components(NULL))
		return E_FAIL;

	Default_Set_Size();
	Default_Set_Delay(0.04f);
...
HRESULT CKatana_Opening::Initialize_Active()
{
	m_iTexture_Index = 0;
	Default_Set_Size();
	Initialize_Set_Scale_Pos_Rotation(NULL);
...
void CKatana_Opening::PriorityTick(_float fTimeDelta)
{
	m_pGraphic_Device->SetRenderState(D3DRS_LIGHTING, false);
}

void CKatana_Opening::Tick(_float fTimeDelta)
{
	...
	if (m_iTexture_Index > m_pTextureCom->Get_MaxTextureNum()) {
		CGameInstance::Get_Instance()->Set_Ui_ActiveState(TEXT("Ui_Katana_Opening"),false);
		CGameInstance::Get_Instance()->Set_Ui_ActiveState(TEXT("Ui_Phone"), true);
...
void CKatana_Opening::LateTick(_float fTimeDelta)
{
...
HRESULT CKatana_Opening::Render()
{
...
void CKatana_Opening::Initialize_Set_Scale_Pos_Rotation(void* pArg)
{
	Set_Ui_Pos(0, -100);
	Set_Divide(1.f);
...
void CKatana_Opening::Initialize_Set_Speed()
{
}

void CKatana_Opening::Default_Set_LifeTime()
{
}

void CKatana_Opening::Default_Set_Size()
{
	m_UiDesc.m_fSizeX = 840;
	m_UiDesc.m_fSizeY = 1514;
}

POINT CKatana_Opening::Get_Texture_Info()
{
...
_float2 CKatana_Opening::Lissajous_Curve(_float _fTimeDelta, _float& _fLissajousTime, _float _fPosX, _float _fPosY, _float _fWitth, _float _fHeight, _float _fLagrangianX, _float _fLagrangianY, _float _fPhaseDelta, _float _fLissajousSpeed)
{
...
HRESULT CKatana_Opening::Add_Components(void* pArg)
{
...
HRESULT CKatana_Opening::Add_Texture(void* pArg)
{
	if (FAILED(Add_Component(LEVEL_STATIC, TEXT("Katana_Opening_Textures")
...
CUi* CKatana_Opening::Create(LPDIRECT3DDEVICE9 pGraphic_Device)
{
	CKatana_Opening* pInstance = new CKatana_Opening(pGraphic_Device);

	if (FAILED(pInstance->Initialize(NULL)))
	{
		MSG_BOX(TEXT("Failed to Created : CKatana_Opening"));

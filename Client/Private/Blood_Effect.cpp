#include "Blood_Effect.h"
#include "GameInstance.h"

CBlood_Effect::CBlood_Effect(LPDIRECT3DDEVICE9 pGraphic_Device)
	: CUi(pGraphic_Device)
{
}

CBlood_Effect::CBlood_Effect(const CBlood_Effect& rhs)
	: CUi(rhs)
{
}

HRESULT CBlood_Effect::Initialize_Prototype()
{

	return S_OK;

}

HRESULT CBlood_Effect::Initialize(void* pArg)
{
	if (E_FAIL == Add_Components(NULL))
		return E_FAIL;

	Default_Set_Size();
	Default_Set_Delay(0.1f);
	Initialize_Set_Scale_Pos_Rotation(NULL);
	Set_Texture_Index(0);



	return S_OK;
}

HRESULT CBlood_Effect::Initialize_Active()
{
	m_iTexture_Index = 0;

	m_iSlashCount = CPlayer_Manager::Get_Instance()->Get_RealSlashCount();


	Alpha_Initialize();
	Default_Set_Size();
	Initialize_Set_Scale_Pos_Rotation(NULL);

	return S_OK;
}

void CBlood_Effect::PriorityTick(_float fTimeDelta)
{
	m_pGraphic_Device->SetRenderState(D3DRS_LIGHTING, false);
}

void CBlood_Effect::Tick(_float fTimeDelta)
{

	if (AnimationDelay(fTimeDelta) < 0.f && m_iTexture_Index <= m_pTextureCom->Get_MaxTextureNum()) {
		m_iTexture_Index++;
		AnimationDelayReset();
	}

	if (m_iTexture_Index > m_pTextureCom->Get_MaxTextureNum()) {
		m_iTexture_Index = 0;
		AnimationDelayReset();
	}

	m_fScale = { Get_Texture_Info().x / m_fDivide , Get_Texture_Info().y / m_fDivide, 1.f };

	if (m_iSlashCount == 0) {
		Set_Ui_Pos(200, 0);
		m_fRotation = { 0.f, 0.f, 90.f };
	}
	else if (m_iSlashCount == 1) {
		Set_Ui_Pos(-100, 100);
		m_fRotation = { 0.f, 0.f, 45.f };
	}
	else if (m_iSlashCount == 2) {
		Set_Ui_Pos(100, 100);
		m_fRotation = { 0.f, 0.f, 135.f };
	}


	m_pTransformCom->Set_Scale(m_fScale);
	m_pTransformCom->Rotation_XYZ(m_fRotation);
	m_pTransformCom->Set_State(CTransform::STATE_POSITION, &_float3(m_UiDesc.m_fX, m_UiDesc.m_fY, 0.f));
}

void CBlood_Effect::LateTick(_float fTimeDelta)
{
	Adjust_Alpha(fTimeDelta);
	m_fDivide += 0.1f * fTimeDelta;
	//_float2 fLissajousPos = Lissajous_Curve(fTimeDelta, m_fLissajousTime, m_UiDesc.m_fX, m_UiDesc.m_fY, 1.5f, 2, 3, 1, 2, 6);
	//m_pTransformCom->Set_State(CTransform::STATE_POSITION, &_float3(m_UiDesc.m_fX + fLissajousPos.x, m_UiDesc.m_fY + fLissajousPos.y, 0.f));
}

void CBlood_Effect::Begin_RenderState()
{
	m_pGraphic_Device->SetTextureStageState(0, D3DTSS_ALPHAOP, D3DTOP_MODULATE);
	m_pGraphic_Device->SetTextureStageState(0, D3DTSS_ALPHAARG1, D3DTA_TEXTURE);
	m_pGraphic_Device->SetTextureStageState(0, D3DTSS_ALPHAARG2, D3DTA_TFACTOR);
	m_pGraphic_Device->SetRenderState(D3DRS_TEXTUREFACTOR, D3DCOLOR_RGBA(255, 255, 255, (int)m_iAlpha));
}

HRESULT CBlood_Effect::Render()
{
	
	if (FAILED(m_pTransformCom->Bind_WorldMatrix()))
		return E_FAIL;

	if (FAILED(m_pTextureCom->Bind_Texture(m_iTexture_Index)))
		return E_FAIL;

	Begin_RenderState();
	m_pVIBufferCom->Render();
	End_RenderState();
	
	return S_OK;
}

void CBlood_Effect::End_RenderState()
{
	m_pGraphic_Device->SetTextureStageState(0, D3DTSS_ALPHAOP, D3DTOP_SELECTARG1);
}

void CBlood_Effect::Initialize_Set_Scale_Pos_Rotation(void* pArg)
{
	Set_Ui_Pos(0, 0);
	Set_Divide(1.0f);

	m_fScale = { m_UiDesc.m_fSizeX / Get_Divide() , m_UiDesc.m_fSizeY / Get_Divide(), 0.f };
	m_fRotation = { 0.f, 0.f, 0.f };

	m_pTransformCom->Set_Scale(m_fScale);
	m_pTransformCom->Rotation_XYZ(m_fRotation);
	m_pTransformCom->Set_State(CTransform::STATE_POSITION, &_float3(m_UiDesc.m_fX, m_UiDesc.m_fY, 0.f));
}

void CBlood_Effect::Initialize_Set_Speed()
{
}

void CBlood_Effect::Default_Set_LifeTime()
{
}

void CBlood_Effect::Default_Set_Size()
{
	m_UiDesc.m_fSizeX = 1280;
	m_UiDesc.m_fSizeY = 720;
}

POINT CBlood_Effect::Get_Texture_Info()
{
	if (m_pTextureCom == nullptr)
		return { 0, 0 };

	return { 1280, 720 };
}

_float2 CBlood_Effect::Lissajous_Curve(_float _fTimeDelta, _float& _fLissajousTime, _float _fPosX, _float _fPosY, _float _fWitth, _float _fHeight, _float _fLagrangianX, _float _fLagrangianY, _float _fPhaseDelta, _float _fLissajousSpeed)
{
	_fLissajousTime += _fTimeDelta * _fLissajousSpeed;

	_fPosX = _fWitth * sin(_fLagrangianX * _fLissajousTime + 3.14f / _fPhaseDelta);
	_fPosY = _fHeight * cos(_fLagrangianY * _fLissajousTime);

	return { _fPosX,_fPosY };
}

void CBlood_Effect::Adjust_Alpha(_float fTimeDelta)
{
	m_iAlpha -= 200.f * fTimeDelta;
	if (m_iAlpha <= 50) m_pGameInstance->Set_Ui_ActiveState(TEXT("Camera_Blood"), false);
}

HRESULT CBlood_Effect::Add_Components(void* pArg)
{
	if (FAILED(Add_Component(
		LEVEL_STATIC,
		TEXT("VIBuffer_Rect_Default"),
		(CComponent**)&m_pVIBufferCom)))
		return E_FAIL;

	if (FAILED(Add_Component(LEVEL_STATIC,
		TEXT("Transform_Default"),
		(CComponent**)&m_pTransformCom)))
		return E_FAIL;

	if (FAILED(Add_Texture(pArg)))
		return E_FAIL;

	return S_OK;
}

HRESULT CBlood_Effect::Add_Texture(void* pArg)
{
	if (FAILED(Add_Component(LEVEL_STATIC, TEXT("Camera_Blood_Textures")
		, (CComponent**)&m_pTextureCom)))
		return E_FAIL;

	return S_OK;

}

CUi* CBlood_Effect::Create(LPDIRECT3DDEVICE9 pGraphic_Device)
{
	CBlood_Effect* pInstance = new CBlood_Effect(pGraphic_Device);

	if (FAILED(pInstance->Initialize(NULL)))
	{
		MSG_BOX(TEXT("Failed to Created : CBlood_Effect"));
		Safe_Release(pInstance);
	}

	return pInstance;
}


void CBlood_Effect::Free()
{
	__super::Free();
}

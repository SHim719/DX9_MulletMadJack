...
void CPhone::LateTick(_float fTimeDelta)
{
    ...
    if (CGameInstance::Get_Instance()->GetKey(eKeyCode::LShift)) {
        ...
        m_pTransformCom->Set_State(CTransform::STATE_POSITION, &_float3(m_UiDesc.m_fX + fOffSet.x, m_UiDesc.m_fY + fDisablePos.x + fOffSet.y, 0.f));
    }
    else {
        ...
        if (CPlayer_Manager::Get_Instance()->Get_Player_MoveState() == CPlayer::MOVE_STATE::MOVE)
            m_pTransformCom->Set_State(CTransform::STATE_POSITION, &_float3(m_UiDesc.m_fX + fLissajousPos.x + fLissajousRun.x , m_UiDesc.m_fY + fLissajousPos.y + fLissajousRun.y + fDisablePos.x, 0.f));
        else
        {
            m_pTransformCom->Set_State(CTransform::STATE_POSITION, &_float3(m_UiDesc.m_fX + fLissajousPos.x, m_UiDesc.m_fY + fLissajousPos.y + fDisablePos.x, 0.f));
        }
    }
    Set_BackGround();
    Set_PhoneNumber();
